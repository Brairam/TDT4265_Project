...
data_flag = 'dermamnist'  # 'pathmnist', 'dermamnist', 'octmnist', 'bloodmnist', 'breastmnist'
...
class MLPMixer(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, img_size=28, patch_size=7, embed_dim=64):
        super().__init__()
    ...
    def forward(self, x):
        x = self.patch_embed(x)  # [B, C, H, W] -> [B, D, P_H, P_W]
        x = x.flatten(2)  # [B, D, P]
...
sns.heatmap(cm, annot=True, fmt='d', cmap='coolwarm', vmax=200, 
...
for i in range(n_classes):
    fpr[i], tpr[i], _ = roc_curve(y_true[:, i], np.array(y_score)[:, i])
...
for i in range(n_classes):
    plt.plot(fpr[i], tpr[i], 
             label=f'{info["label"][str(i)]} (AUC = {roc_auc[i]:.2f})')  # Use str(i) as key
plt.plot([0, 1], [0, 1], 'k--')
